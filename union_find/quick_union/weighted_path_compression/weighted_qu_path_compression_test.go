...
func TestWeightedUnionPathCompression_BetweenTwoTreesOfDifferentSize_ConnectsRootOfSmallerToLargerAndCompresses(t *testing.T) {
	expectedTree := Tree{0, 0, 2, 5, 0, 4, 6, 9, 8, 0}
	actualTree := Tree{0, 0, 2, 5, 4, 4, 6, 9, 8, 1}
	sizeArray := make(SizeArray, len(expectedTree))

	sizeArray[0] = 4
	sizeArray[4] = 3
	err := actualTree.WeightedUnionPathCompression(9, 5, sizeArray)
	require.NoError(t, err)

	assert.Equal(t, expectedTree, actualTree)
}

func TestWeightedUnionPathCompression_BetweenTwoTreesOfSameSize_ConnectsRootOfOneToAnotherAndCompresses(t *testing.T) {
	expectedTree := Tree{4, 0, 2, 5, 4, 4, 6, 7, 8, 0}
...
func TestWeightedUnionPathCompression_BetweenTwoTrees_ConnectsRootOfOneToAnotherAndCompresses(t *testing.T) {
	expectedTree := Tree{7, 1, 1, 1, 2, 2, 5, 1, 11, 7, 1, 10, 10, 10}
	actualTree := Tree{9, 1, 1, 1, 2, 2, 5, 3, 11, 7, 10, 10, 13, 10}
	sizeArray := make(SizeArray, len(expectedTree))

	sizeArray[1] = 10
	sizeArray[10] = 5
	err := actualTree.WeightedUnionPathCompression(0, 12, sizeArray)
	require.NoError(t, err)

	assert.Equal(t, expectedTree, actualTree)
}

func TestWeightedUnionPathCompression_WhenTreesAreAlreadyConnected_ReturnsError(t *testing.T) {
...
func TestWeightedUnionPathCompression_WhenTreeEmpty_ReturnsError(t *testing.T) {
	actualTree := Tree{}
...
func TestWeightedUnionPathCompression_WhenSizeArrayEmpty_ReturnsError(t *testing.T) {
